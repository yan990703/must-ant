    ...
    <a-table :columns="columns" :data-source="data" :pagination="false">
      ...
      <a-space slot="members" slot-scope="members" :size="-12" class="avatar-chips">
        <template v-for="member in members">
          <a-avatar :key="member" :src="member" size="small"/>
        </template>
      </a-space>

      <template slot="company" slot-scope="company">
        <h6 class="m-0">
          <img :src="company.logo" style="margin-right: 10px;" width="25">
          {{ company.name }}
        </h6>
      </template>
      ...
      <template slot="completion" slot-scope="completion">
        <div class="progress-right">
          ...
          <a-progress :percent="completion.value || completion.value == 0 ? completion.value : completion" :show-info="false"
                      :status="completion.status ? completion.status : 'normal'" class="m-0" size="small"/>
        </div>
      </template>

      <template slot="editBtn" slot-scope="row">
        <a-button :data-id="row.key" type="link">
