  ...
  <a-card :bodyStyle="{padding: 0,}" :bordered="false" class="header-solid h-full">
    ...
    <a-table :columns="columns" :data-source="data" :pagination="false">
      ...
      <template slot="company" slot-scope="company">
        <h6 class="m-0">
          <img :src="company.logo" class="mr-10" width="25">
          {{ company.name }}
        </h6>
      </template>
      ...
      <template slot="completion" slot-scope="completion">
        ...
        <a-progress :percent="completion.value ? completion.value : completion" :show-info="false" :status="completion.status ? completion.status : 'normal'"
                    size="small"/>
      </template>
